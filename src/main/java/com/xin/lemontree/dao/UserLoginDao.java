...
/**
 * @author creator mafh 2018/1/17 18:06
 * @author updater mafh
 * @version 1.0.0
 * @description 用户登录dao。
 */
public interface UserLoginDao extends PagingAndSortingRepository<UserLoginEntity, Integer>, JpaSpecificationExecutor<UserLoginEntity> {
     ...
     * @return 用户信息
     */
    UserLoginEntity findByAccount(String account);
     ...
     * @return 用户信息列表
     */
    List<UserLoginEntity> findByEmailEndingWithAndCreateTimeLessThan(String email, Date createTime);
     ...
     * @return 用户数量
     */
    @Query(value = "SELECT COUNT(ul.id) FROM UserLoginEntity ul WHERE ul.platform = :platform AND modifyTime >= :modifyTime")
    long getActiveUserCount(@Param("platform") String platform, @Param("modifyTime") Date modifyTime);
     ...
     * @return 用户信息列表
     */
    @Query(value = "SELECT ul FROM UserLoginEntity ul WHERE ul.email LIKE %?1% OR ul.phone LIKE %?2%")
    List<UserLoginEntity> findByEmailAndPhoneLike(String email, String phone);
     ...
     */
    @Modifying
    @Query(value = "UPDATE UserLoginEntity SET email = :email WHERE id = :id")
